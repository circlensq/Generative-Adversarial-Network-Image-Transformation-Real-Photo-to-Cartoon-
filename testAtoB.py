...
# parser.add_argument("--log", type=str, default='', help="name of log folder")
parser.add_argument("-p", "--hparams", type=str, default='cifar', help="hparams config file")
parser.add_argument('--project_name', required=False, default='GAN',  help='project name')
...
if not os.path.isdir(os.path.join(opts.project_name + '_results', 'Test_AtoB_model_500_train_folder')):
    os.makedirs(os.path.join(opts.project_name + '_results', 'Test_AtoB_model_500_train_folder'))
...
test_photo_loader_src = data_load(os.path.join('./../../../data/', 'faces_bg_removed/'), 'test_filtered', src_transform, batch_size=1, shuffle=False, drop_last=True)
...
with torch.no_grad():
    ...
    for n, (x_a, _) in enumerate(test_photo_loader_src):
        ...
        path = os.path.join(opts.project_name + '_results', 'Test_AtoB_model_500_train_folder/' + str(n+1) + '.png')
